...
from perception_tools.messages.image import Image, Encoding
...
class ZedCamera(CameraInterface):
    def __init__(self, config: ZedCameraConfig, camera_topic_config: CameraTopicConfig) -> None:
        ...
        self.init_params.sdk_verbose = 5
    ...
    def poll(self) -> CameraData | None:
        ...
        camera_data = CameraData(
            color_image=Image(header, color_image_data, encoding=Encoding.RGB8),
...
if __name__ == "__main__":
    rospy.init_node("ros_viewer")
    image_pub = rospy.Publisher("/camera_2/rgb/image", RosImage, queue_size=1)
    info_pub = rospy.Publisher("/camera_2/rgb/camera_info", CameraInfo, queue_size=1)
    point_cloud_pub = rospy.Publisher("/camera_2/point_cloud", PointCloud2, queue_size=1)
    camera = ZedCamera(ZedCameraConfig(), CameraTopicConfig("/camera_2", "camera_2"))
    camera.open()
    while not rospy.is_shutdown():
        data = camera.poll()
        if data is None:
            continue
        image_pub.publish(data.color_image.to_msg())
        point_cloud_pub.publish(data.point_cloud.to_msg())
        info_pub.publish(data.camera_info)
