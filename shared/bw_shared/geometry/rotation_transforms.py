...
def euler_matrix(roll: float, pitch: float, yaw: float, axes: AXIS_KEY = "sxyz") -> np.ndarray:
    """
...
def euler_from_matrix(matrix: np.ndarray, axes: AXIS_KEY = "sxyz") -> tuple[float, float, float]:
    """
    Ripped from tf_conversions.transformations for portability.
...
def quaternion_matrix(quaternion: Quaternion) -> np.ndarray:
    ...
    """
    q = np.array((quaternion.x, quaternion.y, quaternion.z, quaternion.w), dtype=np.float64, copy=True)
    nq = np.dot(q, q)
    if nq < _EPS:
...
def euler_from_quaternion(quaternion: Quaternion, axes: AXIS_KEY = "sxyz"):
    """
    Ripped from tf_conversions.transformations for portability.
