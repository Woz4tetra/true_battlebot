...
def make_aruco_board(config: BoardConfig) -> aruco.Board:
    config = config
    aruco_dict = aruco.getPredefinedDictionary(DICT_MAPPING[config.tag_family])

    if config.board_type == BoardType.CHARUCO:
    ...
    elif config.board_type == BoardType.ARUCO_GRID:
        ...
        raise ValueError(f"Unknown board type: {config.board_type}")

    return board


def generate_image(config: BoardConfig, board: aruco.Board) -> np.ndarray:
    all_marker_size_px = config.num_rows * max(config.square_size, config.marker_size) * config.px_per_meter
    margin_size = int(config.texture_size - all_marker_size_px) // 2
    if margin_size < 0:
        raise ValueError("Texture size is too small for the given board size")
    image = board.generateImage(
        (config.texture_size, config.texture_size), borderBits=config.border_bits, marginSize=margin_size
    )
    image = np.rot90(image, config.num_90_rotations)
    return image
