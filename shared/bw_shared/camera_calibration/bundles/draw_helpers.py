...
def draw_bundle(info: CameraInfo, image: np.ndarray, bundle: BundleResult) -> np.ndarray:
    ...
    for tag_id, tag_point in bundle.tag_poses.items():
        tag_size = bundle.get_config(tag_id).tag_size
        draw_detection(info, draw_image, tag_id, bundle.detection.get_corners(tag_id), tag_point, tag_size)
