...
def load_image_from_bag(bag: Bag, skip_interval: float) -> Generator[np.ndarray, None, None]:
...
def load_image_from_video(video_path: str, skip_interval: float) -> Generator[np.ndarray, None, None]:
...
def load_images_from_dir(image_dir: str) -> Generator[np.ndarray, None, None]:
...
def load_images(image_source: str, skip_interval: float) -> Generator[np.ndarray, None, None]:
    if os.path.isfile(image_source):
        if image_source.endswith(".bag"):
            bag = Bag(image_source)
            images = load_image_from_bag(bag, skip_interval)
            bag.close()
        else:
            images = load_image_from_video(image_source, skip_interval)
    elif os.path.isdir(image_source):
        images = load_images_from_dir(image_source)
    else:
