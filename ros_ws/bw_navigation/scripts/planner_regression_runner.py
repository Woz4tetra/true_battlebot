...
class AppData:
    ...
    message_queue: list[tuple[str, Any]] = field(default_factory=list)
...
class RunConfig:
    initial_pose: Pose2D
    goal_pose: Pose2D
    linear_pid: PidConfig = PidConfig(kp=10.0, ki=0.0, kd=0.0, kf=1.0)
    angular_pid: PidConfig = PidConfig(kp=10.0, ki=1.0, kd=0.0, kf=1.0)
...
def feedback_cb(app: AppData, feedback: GoToGoalFeedback) -> None:
    ...
    with app.lock:
        ...
        app.message_queue.append(("expected_trajectory", feedback.trajectory))
...
def wait_for_trajectory(app: AppData, simulation_controller: SimulationController) -> bool:
    ...
    with app.lock:
        ...
        app.sequence += 1
        app.message_queue.append(("recorded_trajectory", app.recorded_trajectory))
        app.message_queue.append(("measured_trajectory", app.measured_trajectory))
        command_trajectory = Trajectory()
        command_trajectory.header.seq = app.sequence
        command_trajectory.twists = app.commands
        app.message_queue.append(("commands", command_trajectory))
    return True
...
def main() -> None:
    ...
    go_to_goal_client.cancel_all_goals()
    ...
    try:
        ...
        with Bag(bag_path, "w") as bag:
            for topic, msg in app.message_queue:
                bag.write(topic, msg)
